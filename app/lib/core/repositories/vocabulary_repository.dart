...
class VocabularyRepository {
  ...
  // Xóa từ vựng vĩnh viễn (hard delete)
  Future<int> permanentlyDeleteVocabulary(int id) async {
    final db = await _databaseHelper.database;
  ...
  // Lấy từ vựng theo mức độ khó
  Future<List<Vocabulary>> getVocabulariesByDifficulty(
      int deskId, int difficultyLevel) async {
    // Field removed; fallback to all vocabularies in desk
    return getVocabulariesByDeskId(deskId);
  }

  // Lấy từ vựng theo mức độ thành thạo
  Future<List<Vocabulary>> getVocabulariesByMasteryLevel(
      int deskId, int minMastery, int maxMastery) async {
    final db = await _databaseHelper.database;
    final List<Map<String, dynamic>> maps = await db.query(
      'vocabularies',
      where:
          'desk_id = ? AND mastery_level >= ? AND mastery_level <= ? AND is_active = ?',
      whereArgs: [deskId, minMastery, maxMastery, 1],
      orderBy: 'mastery_level DESC, created_at DESC',
    );
  ...
  Future<List<int>> importVocabularies(List<Vocabulary> vocabularies) async {
    final db = await _databaseHelper.database;
    final List<int> insertedIds = [];

    await db.transaction((txn) async {
      for (final vocabulary in vocabularies) {
        final id = await txn.insert('vocabularies', vocabulary.toMap());
        insertedIds.add(id);
      }
    });
